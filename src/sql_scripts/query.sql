...
SELECT *
FROM unit AS u
	JOIN market AS m ON m.unit_id = u.id;
...
SELECT unit_storage.wood AS unit_wood,
	unit_storage.food AS unit_food,
	unit_storage.gold AS unit_gold,
	unit_storage.stone AS unit_stone
FROM unit AS unit
...
SELECT SUM(s.wood) AS total_wood,
	SUM(s.gold) AS total_gold,
	SUM(s.food) AS total_food,
	SUM(s.stone) AS total_stone
FROM storage AS s
	LEFT JOIN unit AS u ON u.storage_id = s.id
	LEFT JOIN equipment AS e ON e.storage_id = s.id
WHERE u.name LIKE 'Spearmen'
	OR e.name LIKE 'Mail armor';
SELECT *
FROM equipment;
SELECT *
FROM storage;
...
FROM unit AS u
	...
	LEFT JOIN skill AS s ON s.id = td.skill_id
	LEFT JOIN equipment AS eq ON eq.id = td.equipment_id